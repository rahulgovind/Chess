...
void Engine::EvaluateAllPlayer1()
{
    ...
    //Just for debugging
    for(int i=1;i<=16;i++)
        if((player1_current[i].first <0 || player1_current[i].second <0 ) && player1_possible[i].size()>0)
            printf("Player1: Error with index: %d\n", i);


    game_status = EvaluateGameState1();
}
...
void Engine::EvaluateAllPlayer2()
{
    ...
    //Just for debugging
    for(int i=1;i<=16;i++)
        if((player2_current[i].first <0 || player2_current[i].second <0 ) && player2_possible[i].size()>0)
            printf("Player2. Error with index: %d\n", i);



    moves ai = minimax_base(4);
    printf("%u\t%u\t%u\t%u\n", ai.x0, ai.y0, ai.x1, ai.y1);

    game_status = EvaluateGameState2();
}
...
//Assuming moves have already been evaluated. Returns if checkmate or stalemate
int Engine::EvaluateGameState1()
{
    bool check = CheckIfCheck1();
    int total_moves = 0;
    for(int i=1;i<=16;i++)
        if(player1_current[i].first>=0)
            total_moves += player1_possible[i].size();

    if(check)
    {
        if(total_moves == 0)
            return CHECKMATE_PLAYER1;
        else
            return CHECK_PLAYER1;
    }
    else
        if(total_moves == 0)
            return STALEMATE;
...
//Assumes moves have already been evaluated. Returns if checkmate or stalemate
int Engine::EvaluateGameState2()
{
    bool check = CheckIfCheck2();
    int total_moves = 0;
    for(int i=1;i<=16;i++)
        if(player2_current[i].first>=0)
            total_moves += player2_possible[i].size();

    if(check)
    {
        if(total_moves == 0)
            return CHECKMATE_PLAYER2;
        else
            return CHECK_PLAYER2;
    }
    else
        if(total_moves == 0)
            return STALEMATE;
